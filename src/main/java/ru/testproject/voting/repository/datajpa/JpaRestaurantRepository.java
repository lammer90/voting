...
public interface JpaRestaurantRepository extends JpaRepository<Restaurant, Integer> {
    ...
    @Query("SELECT DISTINCT r FROM Restaurant r LEFT JOIN FETCH r.dishes d LEFT JOIN FETCH r.votes v WHERE v.date=:date AND d.date=:date ORDER BY r.name, d.name")
